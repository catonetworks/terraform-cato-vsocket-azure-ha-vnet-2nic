locals {
  ...
  wan_subnet_name_local             = var.wan_subnet_name != null ? var.wan_subnet_name : "${var.resource_prefix_name}-subnetWAN"
  lan_subnet_name_local             = var.lan_subnet_name != null ? var.lan_subnet_name : "${var.resource_prefix_name}-subnetLAN"
  ha_identity_name_local            = var.ha_identity_name != null ? var.ha_identity_name : "${var.resource_prefix_name}-CatoHaIdentity"
  vsocket_primary_disk_name_local   = var.vsocket_primary_disk_name != null ? var.vsocket_primary_disk_name : "${var.resource_prefix_name}-vSocket-disk-primary"
  vsocket_secondary_disk_name_local = var.vsocket_secondary_disk_name != null ? var.vsocket_secondary_disk_name : "${var.resource_prefix_name}-vSocket-disk-secondary"
}
